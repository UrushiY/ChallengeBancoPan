...
class ClienteController:
    ...
    def autenticarAws(self, cpf, senha):
        try:
            with Aws.Aws.connect() as conn:
                sql = 'SELECT * FROM clientes WHERE cpf=:1'
    ...
    def pesquisarDadosPan(cpf):
        try:
            sql = 'SELECT JSON_OBJECT(*) FROM clientes WHERE cpf=:1 ORDER BY codCliente DESC'
            with Pan.Oracle.connect() as conn:
    ...
    def cadastrarAws(cpf, nome, sobrenome, rg, dataNasc, genero, profissao, renda, estadoCiv, etnia, email, senha):
        try:
            ...
            with Aws.Aws.connect() as conn:
                ...
                with conn.cursor() as cursor:
                    cursor.execute(sql, [cpf, nome, sobrenome, rg, dataNasc,
                                   genero, profissao, renda, estadoCiv, etnia, email, senha])
